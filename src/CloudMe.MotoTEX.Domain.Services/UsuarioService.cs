...
namespace CloudMe.MotoTEX.Domain.Services
{
    public class UsuarioService : ServiceBase<Usuario, UsuarioSummary, Guid>, IUsuarioService
    {
        ...
        private readonly IIdentityService<CloudMeMotoTEXContext, UserDto<Guid>, Guid, RoleDto<Guid>, Guid, Guid, Guid, CloudMe.MotoTEX.Infraestructure.Entries.Usuario,IdentityRole<Guid>, Guid, IdentityUserClaim<Guid>, IdentityUserRole<Guid>,  IdentityUserLogin<Guid>, IdentityRoleClaim<Guid>, IdentityUserToken<Guid>> _identityService;
        UserManager<Usuario> _userManager;

        public UsuarioService(
            IUsuarioRepository userRepository,
            UserManager<Usuario> userManager,
            IIdentityService<CloudMeMotoTEXContext, UserDto<Guid>, Guid, RoleDto<Guid>, Guid, Guid, Guid, CloudMe.MotoTEX.Infraestructure.Entries.Usuario,IdentityRole<Guid>, Guid, IdentityUserClaim<Guid>, IdentityUserRole<Guid>,  IdentityUserLogin<Guid>, IdentityRoleClaim<Guid>, IdentityUserToken<Guid>> identityService)
        {
        ...
        public override async Task<Usuario> CreateAsync(UsuarioSummary summary)
        {
            ...
            var usrEmail = await _userManager.FindByEmailAsync(summary.Email);
        ...
        public override async Task<Usuario> UpdateAsync(UsuarioSummary summary)
        {
            try
            {
                ...
                var usrEmail = await _userManager.FindByEmailAsync(summary.Email);
                if (usrEmail != null && !string.IsNullOrEmpty(summary.Email) && usrEmail.Id != summary.Id)
                {
                    AddNotification("Usuários", string.Format("Email '{0}' está sendo utilizado por outro usuário", summary.Email));
                }
